# scripts/clean_ls_filenames.py
# Label Studio文件名清理工具，批量去除文件名前的随机UUID前缀，恢复原始DJI_开头的命名格式
...
# 扫描指定目录下的子文件夹，寻找符合Label Studio命名特征的文件，并将其重命名为原始格式
def clean_ls_prefix():
    # 检查目标路径是否存在
    ...
    for sub in SUB_FOLDERS:
        ...
        for filename in os.listdir(sub_path):
            # 匹配Label Studio的前缀特征：包含横杠且后面跟着DJI
